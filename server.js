...
// 🗄️ เชื่อมต่อฐานข้อมูล (สลับระหว่าง SQLite และ PostgreSQL อัตโนมัติ)
let db;
...
if (process.env.DATABASE_URL) {
    ...
    class PostgresWrapper {
        ...
        mapRowKeys(row) {
            ...
            for (let key in row) {
                if (row.hasOwnProperty(key)) {
                    ...
                    if (keyMap[lowerKey]) {
                        mapped[keyMap[lowerKey]] = row[key];
                    } else {
                        mapped[key] = row[key];
                    }
                }
        ...
        translate(sql) {
            ...
            // 2. แปลงคำสั่งประเภทข้อมูลและคีย์หลักเฉพาะตัวของ SQLite
            translated = translated.replace(/INTEGER PRIMARY KEY AUTOINCREMENT/gi, 'SERIAL PRIMARY KEY');
            translated = translated.replace(/AUTOINCREMENT/gi, '');
            
            // 3. แปลง INSERT OR IGNORE INTO teachers
            if (/INSERT OR IGNORE INTO teachers/i.test(translated)) {
                translated = translated.replace(/INSERT OR IGNORE INTO/i, 'INSERT INTO') + ' ON CONFLICT (username) DO NOTHING';
            }
            // 4. แปลง INSERT OR IGNORE INTO teacher_rooms
            else if (/INSERT OR IGNORE INTO teacher_rooms/i.test(translated)) {
                translated = translated.replace(/INSERT OR IGNORE INTO/i, 'INSERT INTO') + ' ON CONFLICT (roomid) DO NOTHING';
            }
            else if (/INSERT OR IGNORE INTO/i.test(translated)) {
                translated = translated.replace(/INSERT OR IGNORE INTO/i, 'INSERT INTO');
            }

            // 5. แปลง INSERT OR REPLACE INTO
            if (/INSERT OR REPLACE INTO/i.test(translated)) {
                translated = translated.replace(/INSERT OR REPLACE INTO/i, 'INSERT INTO');
            }
        ...
        serialize(callback) {
            this.inSerialize = true;
            this.serializeQueue = [];
            
            // เรียกใช้ callback ซึ่งจะบันทึกรันคำสั่ง SQL ต่างๆ ลงคิวพร้อมกันแบบ sync
            callback();
            
            this.inSerialize = false;
            
            // ดำเนินการรันทีละคิวแบบ sequential
            const executeNext = () => {
                if (this.serializeQueue.length === 0) return;
                const nextQuery = this.serializeQueue.shift();
                nextQuery().then(() => {
                    executeNext();
                });
            };
        ...
        run(sql, params, callback) {
            ...
            const execute = () => {
                return new Promise((resolve) => {
                    ...
                    this.pool.query(translatedSql, params, (err, res) => {
                        if (callback) {
                            try {
                                const context = {
            ...
            if (this.inSerialize) {
                this.serializeQueue.push(execute);
            } else {
                execute();
            }
        }
        ...
        get(sql, params, callback) {
            ...
            const execute = () => {
                return new Promise((resolve) => {
                    ...
                    this.pool.query(translatedSql, params, (err, res) => {
                        if (callback) {
                            let row = res && res.rows && res.rows.length > 0 ? res.rows[0] : undefined;
                            if (row) row = this.mapRowKeys(row);
                            try {
                                callback(err, row);
                            } catch (e) {
                                console.error("Error in callback:", e);
                ...
                return new Promise((resolve) => {
                    ...
                    this.pool.query(translatedSql, params, (err, res) => {
                        if (callback) {
                            ...
                            rows = rows.map(r => this.mapRowKeys(r));
                            try {
                                callback(err, rows);
                            } catch (e) {
                                console.error("Error in callback:", e);
        ...
        prepare(sql) {
            ...
            return {
                run(...args) {
                    ...
                    const execute = () => {
                        return new Promise((resolve) => {
                            self.pool.query(translatedSql, params, (err) => {
                                if (callback) {
                                    try {
                                        callback(err);
                                    } catch (e) {
                    ...
                    if (self.inSerialize) {
                        self.serializeQueue.push(execute);
                    } else {
                        execute();
                    }
                },
    ...
    db = new PostgresWrapper(process.env.DATABASE_URL);
    console.log('🟢 เชื่อมต่อฐานข้อมูล Neon (PostgreSQL) บนออนไลน์สำเร็จ');
} else {
    // 🗄️ เชื่อมต่อฐานข้อมูล SQLite ในเครื่อง (สร้างโฟลเดอร์ .data สำหรับเครื่องโลคอล)
